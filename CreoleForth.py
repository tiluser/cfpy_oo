...
import sys
# sys.path.append("C:\\Users\\tilus\\AppData\\Roaming\\LibreOffice\\4\\user\\Scripts\\python")
...
class CorePrims:
    ...
    def doHello(self, gsp):
        ...
        print(myBox.show("Hello",0,"Dialog title"))
    ...
    def doTulip(self, gsp):
        ...
        myBox.renderFromButtonSize()
        myBox.numberOflines = 2
        print(myBox.show("Tulip",0,"Creole Forth"))
...
class AppSpec:
    ...
    def doTest(self, gsp):
        ...
        myBox.renderFromButtonSize()
        myBox.numberOflines = 2
        print(myBox.show("Testing definition - put whatever you want here",0,"Dialog title"))
        return 0
        
    #  ( flag -- ) Turns LED 13 on the Arduino Uno on or off
    def doLed13(self, gsp):
        flag = int(gsp.DataStack.pop())
        port = "/dev/ttyUSB0"
        ser = s.Serial(port, 9600, timeout=0)   # check your com port
        t.sleep(2)
        if (flag == 0):
            ser.write(b'0')
        else:
    ...
    # ( -- ) Turns LED 13 on the Arduino Uno off
    def doLed13Off(self, gsp):
        port = "/dev/ttyUSB0"
        ser = s.Serial(port, 9600, timeout=0)   # check your com port
        ser.write(b'0')
        return 0
...
class CreoleWord:
    def __init__(self, NameField, CodeField, CodeFieldStr, Vocabulary, 
    CompileActionField, HelpField,PrevRowLocField, RowLocField, LinkField, IndexField, ParamField, DataField):
...
#cfb1.buildPrimitive("OPENSER", cfb1.Modules.AppSpec.doOpenSerial, "AppSpec.doOpenSerial", "APPSPEC", "COMPINPF","( -- ) Establishes the connection to the serial port")
#cfb1.buildPrimitive("CLOSESER", cfb1.Modules.AppSpec.doCloseSerial, "AppSpec.doCloseSerial", "APPSPEC", "COMPINPF","( -- ) Closes the connection to the serial port")
...
#cfb1.buildPrimitive("CALL_BLINK", cfb1.Modules.AppSpec.doCallBlink, "AppSpec.doCallBlink", "APPSPEC", "COMPINPF","( -- ) Calls the Arduino blink routine example")
cfb1.buildPrimitive("LED13", cfb1.Modules.AppSpec.doLed13, "AppSpec.doLed13", "APPSPEC", "COMPINPF","( flag -- ) Turns LED 13 on the Arduino Uno on or off")
cfb1.buildPrimitive("LED13OFF", cfb1.Modules.AppSpec.doLed13Off, "AppSpec.doLed13Off", "APPSPEC", "COMPINPF","( flag -- ) Turns LED 13 on the Arduino Uno off")
...
def doit(myargs):
    for each_number in range(20): 
        ...
        myBox.renderFromButtonSize()
        myBox.numberOflines = 2
        print(myBox.show(str(active_sheet.getCellByPosition(1, each_number).getFormula()),0,"Dialog title"))
        """
        if active_sheet.getCellByPosition(2, each_number).getFormula().upper() == "X":           
            gsp.InputArea = active_sheet.getCellByPosition(1, each_number).getFormula()
